...
titanic_forest<-function(){
        ...
        fit <- randomForest(Survived ~ Pclass + Sex + Age + Child + Sex*Pclass + Family + Mother, data=trainData)
        print(fit)
        importance(fit)
        ...
        resultY <- predict(fit,testData,response="Decesion")
        Survival <- vector()
        for(i in 1:length(resultY)){
                if(resultY[i]>0.50){
                        Survival[i]=1
                }else{
                        Survival[i]=0
        ...
        write.csv(kaggle.sub,file = "kaggle_forest1.csv", row.names = FALSE)
