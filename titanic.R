titanic <- function()
{
        ...
        titanic_train <- function(){
                
                trainData <- read.csv("train.csv",header=TRUE,stringsAsFactor=FALSE)
                trainData <- trainData[-c(1,9:12)]
                
                
                trainData$Sex = gsub(pattern = "female",replacement = 1,x = trainData$Sex)
                trainData$Sex = gsub(pattern = "male",replacement = 0,x = trainData$Sex)
        ...
        passengerTest <- read.csv("test.csv",header = TRUE)
        passengerID <- passengerTest[1]
        trainX <- as.matrix(as.numeric(trainData$Pclass),as.numeric(trainData$Sex),as.numeric(trainData$Age),as.numeric(trainData$Family),as.numeric(trainData$Mother))
        trainY <- as.matrix(trainData$Survived)
        
        testX <-  as.matrix(as.numeric(testData$Pclass),as.numeric(testData$Sex),as.numeric(testDate$Age),as.numeric(testData$Family),as.numeric(testData$Mother))
        ...
        svp <- ksvm(trainX,trainY,type="C-svc",kernel="rbf",C=1)
        testY <- predict(svp,testX)
        survival <- testY
        kaggle.sub <- cbind(passengerID,survival)
        colnames(kaggle.sub) <- c("PassengerId","Survived")
        write.csv(kaggle.sub,file = "kaggle_svm.csv", row.names = FALSE)
        ##testData
}